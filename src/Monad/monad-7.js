...
const validateTitleLength = (title) => {
  return title.length > 30 ? Maybe.of(null) : Maybe.of(title);
}
...
const logBookAuthor = (bookId, books)  => {
  return pipe(
    ...
    map(prop('title')), // Either('coding with javascript')
    chain(validateTitleLength), // Maybe('coding with javascript')
    map(toUpper), // Maybe('COADING WITH JAVASCRIPT')
  )(books);
}

// Maybe('COADING WITH JAVASCRIPT')
